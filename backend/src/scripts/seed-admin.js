...
export async function seedAdmin() {
  try {
    ...
    if (lookupError) {
      console.error('❌  Lỗi kiểm tra tài khoản admin tồn tại:', lookupError.message);
      return;
    }
    ...
    if (existing) {
      ...
      const passwordMatch = existing.password_hash
        ? await bcrypt.compare(ADMIN_DEFAULT_PASSWORD, existing.password_hash)
        : false;
    ...
    console.error('❌  Lỗi nghiêm trọng trong quá trình seed admin:', err);
  }
}
