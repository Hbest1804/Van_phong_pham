...
// Production (Docker/Nginx): VITE_API_URL không set → dùng origin hiện tại (relative)
// Development: VITE_API_URL=http://localhost:5000
const _base = import.meta.env.VITE_API_URL ?? '';
const BASE_URL = (_base !== '' ? _base : '') + '/api/v1';
...
export const adminUsersApi = {
   ...
   * Lấy danh sách tài khoản khách hàng (role = 'user')
   */
  getUsers: (params: GetUsersParams = {}) => {
    ...
    const qs = query.toString();
    return request<PaginatedUsers>(`/admin/users${qs ? `?${qs}` : ''}`, { method: 'GET' });
  },

  /**
   * GET /api/v1/admin/users/:id
   * Chi tiết một tài khoản khách hàng
   */
